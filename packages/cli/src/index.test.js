...
describe('@emdaer/cli', () => {
  ...
  test('logs warning a passed path does not exist', async () => {
    glob.mockReturnValue([]);
    fs.readJson.mockImplementationOnce(() => ({
      name: '@emdaer/cli',
    }));
    ...
    const exitCode = await cli(jest.fn().mockImplementation(() => ''), [
      ...
      '.emdaer/CONTRIBUTED.emdaer.md',
    ]);
    expect(logger.warn).toHaveBeenLastCalledWith(
      NO_MATCHING_FILES('.emdaer/CONTRIBUTED.emdaer.md')
    );
    expect(exitCode).toBe(0);
  });
  test('logs warning all passed path does not exist', async () => {
    glob.mockReturnValue([]);
    fs.readJson.mockImplementationOnce(() => ({
      name: '@emdaer/cli',
    }));
    ...
    expect(logger.warn).toHaveBeenLastCalledWith(
      NO_MATCHING_FILES(
        '.emdaer/README.emdaer.md, .emdaer/CONTRIBUTED.emdaer.md'
      )
    );
    expect(exitCode).toBe(0);
  });
  test('logs error when emdaer fails', async () => {
    ...
    glob.mockReturnValue(['README.emdaer.md']);
    fs.readJson.mockImplementationOnce(() => ({
      name: '@emdaer/cli',
    }));
    fs.readFile.mockImplementation(() => '');
  ...
  test('warns when overwriting was skipped', async () => {
    glob.mockReturnValue(['lol/cool/nested/paths/README.emdaer.md']);
    fs.readJson.mockImplementationOnce(() => ({
      name: '@emdaer/cli',
    }));
    ...
    const exitCode = await cli(emdaerMock);
    expect(logger.warn).toHaveBeenLastCalledWith(
      'skipping lol/cool/nested/paths/README.md for @emdaer/cli ↩️'
    );
    expect(exitCode).toBe(0);
  });
  test('overwrites when prompt is answered true', async () => {
    glob.mockReturnValue(['.emdaer/nested/ignored/README.emdaer.md']);
    fs.readJson.mockImplementationOnce(() => ({
      name: '@emdaer/cli',
    }));
    fs.readFile
      .mockImplementationOnce(() => 'emdaerHash:doesnotmatch')
  ...
  Its template can be found at .emdaer/nested/ignored/README.emdaer.md
-->
